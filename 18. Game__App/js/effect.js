...
$(document).ready(function () {
    ...
    function LimitApp(device) {
        ...
        function ChangeColor_Status(x) {
            if (($('.SaveStyle--' + device + ' .touchsave' + x).html() == 'true' + x) || ($('.SaveStyle--' + device + ' .locksave' + x).html() == 'true' + x) || ($('.SaveStyle--' + device + ' .clocksave' + x).html().replace(/\s/g, '') == 'true' + x)) {
                $('.day-item--' + device + ':nth-child(' + (x - 1) + ') div').css('color', $('.savecolor').html().replace(/\s/g, ''));
                $('.day-item--' + device + ':nth-child(' + (x - 1) + ')').css('border', '1px solid ' + $('.SaveStyle--' + device + ' .savecolor').html().replace(/\s/g, '') + '');
    ...
    function Access__App() {
        ...
        } else if ($('.ac-App').prop('checked') == true) {
    ...
    function SetContentReponsive() {
        ...
        else if (height >= 400 && height <= 685 && width > 800) {
            $('.content-TimeLimit').css('height', height / 1.18 + 30);
        }
        else if (height > 685 && height <= 720 && width > 800) {
            $('.content-TimeLimit').css('height', height / 1.18 + 30);
        }
...
///Hide Space white of web-filter when categories show
